...
class Results(Page):
    def vars_for_template(self):
        ...
        for i in range(a):
            ...
            win = True if self.player.in_round(i + 1).bid < self.player.in_round(i+1).bot_bid else False
            price = self.player.in_round(i + 1).bid if win else self.player.in_round(i+1).bot_bid
        ...
        return dict(results=result_list, a=a,bid=bid,win=win,price=price,profit=profit,winner=winner,payoff=payoff,others_bid=others_bid)
